...
6. [Category 4: Prediction Accuracy Tracking](#category-4-prediction-accuracy-tracking)
7. [Edge Function Reference](#edge-function-reference)
...
AI Royale processes four distinct categories of data:
...
| **Prediction Tracking** | Battle outcomes linked to predictions | Formula-based, auditable | Prediction accuracy, error rates |
...
## Category 4: Prediction Accuracy Tracking

Data linking AI predictions to actual battle outcomes for accuracy measurement.

### Matchup Prediction Caching

**Edge Function:** `predict-deck-matchup/index.ts`  
**Model:** `google/gemini-2.5-flash`  
**Caching:** 7 days, keyed by `deck_hash` (sorted card names concatenated)

| Field | Description | Storage Location |
|-------|-------------|------------------|
| `predicted_win_rate_a` | AI predicted win % for deck A | `matchup_predictions` |
| `predicted_win_rate_b` | AI predicted win % for deck B | `matchup_predictions` |
| `confidence` | Prediction confidence level | `matchup_predictions` |
| `key_matchups` | Critical card interactions | `matchup_predictions` |
| `tips` | Strategic recommendations | `matchup_predictions` |

### Battle Outcome Linking

**Location:** `track-deck-stats/index.ts`

When battles sync, the system links outcomes to cached predictions:

```typescript
// Deck hash matching
const deckHashA = [...playerCards].sort().join('_').toLowerCase();
const deckHashB = [...opponentCards].sort().join('_').toLowerCase();
const combinedHash = [deckHashA, deckHashB].sort().join('|');

// Find matching prediction
const { data: prediction } = await supabase
  .from('matchup_predictions')
  .select('*')
  .eq('deck_hash', combinedHash)
  .eq('player_tag', playerTag)
  .single();

// Update with actual outcome
if (prediction && battleTime > prediction.last_battle_at) {
  await supabase
    .from('matchup_predictions')
    .update({
      actual_wins_deck_a: isWin ? prediction.actual_wins_deck_a + 1 : prediction.actual_wins_deck_a,
      actual_losses_deck_a: isWin ? prediction.actual_losses_deck_a : prediction.actual_losses_deck_a + 1,
      actual_battles_total: prediction.actual_battles_total + 1,
      last_battle_at: battleTime
    })
    .eq('id', prediction.id);
}
```
...
| Metric | Formula | Storage | Display Location |
|--------|---------|---------|------------------|
| Actual Win Rate | `actual_wins_deck_a / actual_battles_total * 100` | Calculated on read | `PredictionAccuracyChart.tsx` |
| Prediction Error | `predicted_win_rate_a - actual_win_rate` | `matchup_predictions.prediction_error` | `PredictionAccuracyChart.tsx` |
| Accuracy Badge | Error-based: <10% Excellent, <20% Good, <30% Fair, ≥30% Poor | Calculated | `PredictionAccuracyChart.tsx` |

### Prediction Trend Analysis

**Location:** `src/components/deck/PredictionAccuracyChart.tsx`

```typescript
// Trend calculation comparing older vs newer prediction errors
function calculateTrend(predictions: PredictionHistoryEntry[]) {
  const withActuals = predictions.filter(p => p.actualBattlesTotal > 0 && p.predictionError !== null);
  if (withActuals.length < 4) return null;
  ...
  if (newerAvgError - olderAvgError <= -5) return 'improving';
  if (newerAvgError - olderAvgError >= 5) return 'worsening';
  return 'stable';
}
```
...
|-------|-----------|---------|
| Improving | Error decreased by ≥5% | Green TrendingUp icon |
| Worsening | Error increased by ≥5% | Red TrendingDown icon |
| Stable | Error change <5% | Gray Minus icon |
...
|----------|---------|---------------|-----------|
| `track-deck-stats` | Aggregate deck performance + link predictions | Yes | `deck_usage_stats`, `matchup_predictions` |
...
| `predict-deck-matchup` | Deck vs deck win rate prediction | Yes | `matchup_predictions` (7 day TTL) |
