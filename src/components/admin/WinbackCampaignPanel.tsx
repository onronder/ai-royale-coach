 ...
 * Allows admins to configure promo codes and send personalized emails.
 */
  ...
  const { data: campaignHistory, isLoading: loadingHistory } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, target_email, promo_code, discount_percent, sent_at')
    ...
    },
    {
      ...
      render: (user) => (
        <Button
          ...
          disabled={!promoCode.trim() || sendEmailMutation.isPending || !canSendToUser(user)}
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Shown in the email (must match Polar discount)
              </p>
            </div>
          </div>

          {(!promoCode.trim() || discountPercent < 1 || discountPercent > 100) && (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <Button
                variant="default"
                disabled={!promoCode.trim() || eligibleToSendCount === 0 || sendingToAll}
      ...
      <Card>
        ...
        <CardContent>
          {loadingHistory ? (
            <ResponsiveTableSkeleton columns={4} rows={5} />
          ) : (
            <ResponsiveTable
