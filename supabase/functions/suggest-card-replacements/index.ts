...
serve(async (req) => {
  ...
  try {
    const { currentDeck, targetCard, availableCards } = await req.json();
    ...
    const prompt = `You are a Clash Royale deck building expert. 
...
Focus on practical, viable replacements that improve the deck.`;
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are a Clash Royale deck building expert.' },
        ...
        tools: [{
          ...
          function: {
            ...
            description: "Return card replacement suggestions",
            parameters: {
              type: "object",
              properties: {
                suggestions: {
